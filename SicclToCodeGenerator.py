...
class SicclToCodeGenerator():
    ...
    def generate_recursive_thread_dependency_graph(self, thread_dgraph_comp: list[int], thread_dgraph_tokenized: dict[int, list[int]], to_look_up_thread: int) -> dict[int, list[int]]:
        if to_look_up_thread in thread_dgraph_tokenized:
            ...
            for i, dp in enumerate(dependant_threads):
                ...
                self.generate_recursive_thread_dependency_graph(thread_dgraph_comp, thread_dgraph_tokenized, dp)
    ...
    def gen_dep_graphs(self, siccl_array):
        ...
        for thread_name, dependant_threads in thread_dgraph_tokenized.items():
            ...
            thread_dgraph[thread_name].extend(self.generate_recursive_thread_dependency_graph(tmp_graph, thread_dgraph_tokenized, thread_name))
    ...
    def generate_thread_params_graph_recursive(self, siccl_array: np.array, dependency_graph: dict[int, list[int]]) -> dict[int, list[int]]:
    ...
    def generate(self, siccl_array: list) -> string:
        self.backend.reset()
        self.reset()
        thread_dgraph, thread_dgraph_recursive = self.gen_dep_graphs(siccl_array)
